...
% Adaptive maximum iteration count.
...
poisson_rel = zeros(localMaxIter,1);
poisson_conv = false(localMaxIter,1);
...
for iter = 1:localMaxIter
    ...
    [Ru(iter),Rv(iter),Rc_mass(iter),Rc_div(iter)] = ...
        velocity_residuals(u,v,u_old,v_old,dx,dy,cfg.U_lid,cfg.L);

    dt_hist(iter) = dt;
    poisson_iters(iter) = pinfo.iter;
    poisson_rel(iter) = pinfo.final_relative_residual;
    poisson_conv(iter) = pinfo.converged;
...
poisson_rel = poisson_rel(1:iter);
poisson_conv = poisson_conv(1:iter);
...
result.Ru = Ru;
result.Rv = Rv;
result.Rc = Rc_mass;          % backward compatible name
result.Rc_mass = Rc_mass;
result.Rc_div = Rc_div;
result.dt = dt_hist;
result.poisson_iters = poisson_iters;
result.poisson_relative_residual = poisson_rel;
result.poisson_converged = poisson_conv;
...
result.final_Rc = Rc_mass(end);
result.final_Rc_mass = Rc_mass(end);
result.final_Rc_div = Rc_div(end);
result.avg_poisson_iters = mean(poisson_iters);
result.avg_poisson_relative_residual = mean(poisson_rel);
result.pressure_saturation_ratio = mean(poisson_iters >= cfg.poisson_maxIter);
